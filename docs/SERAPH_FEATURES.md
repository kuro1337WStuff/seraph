...
| instruction-info | done | `ZydisInstructionInfo` | `push`, `cpuid`, `imul`, `call`, `ret`, `fld`, and `xsave` report implicit registers, flags, flow, and stack or FPU delta. GPR views are included, and so are `xmm`/`ymm`/`zmm` (upper bits zeroed on a write), `mm`/`st` aliases, and the `flags`/`eflags`/`rflags` and `ip`/`eip`/`rip` ladders. |
